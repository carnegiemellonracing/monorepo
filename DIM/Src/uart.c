...
/** @brief UART 200 Hz TX priority. */
static const uint32_t uartTX200Hz_priority = 3;

/** @brief UART 200 Hz TX period (milliseconds). */
static const TickType_t uartTX200Hz_period_ms = 5;

/** @brief UART 200 Hz TX task. */
static cmr_task_t uartTX200Hz_task;
...
/**
 * @brief Task for sending UART messages at 200 Hz.
 *
 * @param pvParameters Ignored.
 *
 * @return Does not return.
 */
static void uartTX200Hz(void *pvParameters) {
    (void)pvParameters;  // Placate compiler.

    TickType_t lastWakeTime = xTaskGetTickCount();
    while (1) {

		cmr_canRTCDate *curDate = getPayload(CANRX_RTC_DATE);
		cmr_canRTCTime *curTime = getPayload(CANRX_RTC_TIME);

		uart_sendMessage(&uart, curDate, sizeof(cmr_canRTCDate));
		uart_sendMessage(&uart, curTime, sizeof(cmr_canRTCTime));

        vTaskDelayUntil(&lastWakeTime, uartTX200Hz_period_ms);
    }
}
