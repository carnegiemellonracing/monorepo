...
static void uartRX_Task(void *pvParameters) {
    ...
    RxBuffer rx[NUM_RX_BUFFERS] = {
        [0 ... NUM_RX_BUFFERS - 1] =
            {
                .bytes_present = 0,
                // .msg and .buf are zero-initialized implicitly (if
                // static/global)
            },
    };
    ...
    while (1) {
        ...
        for (size_t i = 0; i < NUM_RX_BUFFERS; i++) {
            ...
            memmove(curr.buf, curr.buf + bytes_parsed,
...
static void handle_command(cn_cbor *command) {
    ...
    if (msg != NULL) {
        ...
        if ((id != NULL &&
            ...
            (data != NULL &&
             (data->type == CN_CBOR_BYTES || data->type == CN_CBOR_MAP))) {
            if ((uint16_t)id->v.uint == CMR_CANID_DIM_TEXT_WRITE) {
                ...
                for (size_t i = 0; i < data->length; i += sizePerMessage) {
                    cmr_canDIMTextWrite_t canData = (cmr_canDIMTextWrite_t){
                        .address = i / sizePerMessage, .data = {0}};
                ...
                if (data->length == sizeof(cmr_canCDCPowerLimit_t)) {
                    ...
                    canTX((cmr_canBusID_t)bus->v.uint, (uint16_t)id->v.uint,
                          powerLimit, sizeof(cmr_canCDCPowerLimit_t), 200);
                }
            } else {
                // Do not allow transmission on tractive CAN.
                if (bus->v.uint == CMR_CAN_BUS_VEH ||
                    bus->v.uint == CMR_CAN_BUS_DAQ) {
                    canTX((cmr_canBusID_t)bus->v.uint, (uint16_t)id->v.uint,
                          data->v.bytes, data->length, portMAX_DELAY);
                }
            }
        }
    }
    ...
    if (params != NULL &&
        ...
        for (size_t i = 0; i < len; i += sizeof(struct param_pair)) {
            struct param_pair *pair =
                (struct param_pair *)(params->v.bytes + i);
    ...
    if (signal_en != NULL &&
        ...
        for (uint32_t i = 0; i < MIN((uint32_t)signal_en->length, MAX_SIGNALS);
             i++) {
    ...
    if (response_num_pairs) {
        ...
        if (formatted_response_data) {
            ...
            (void)cn_cbor_mapput_string(response_packet, "params",
...
void uartInit(void) {
    ...
    cmr_uartInit(&uart.port, USART1, &uartInit, GPIOA, GPIO_PIN_10, /* rx */
                 GPIOA, GPIO_PIN_9,                                 /* tx */
                 DMA2_Stream5,
