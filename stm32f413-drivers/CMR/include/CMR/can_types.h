...
typedef struct {
    ...
    uint8_t error[2];     //Flag:cmr_canVSMHeartbeatErr_t cmr_canDIMHeartbeatErr_t cmr_canVCUHeartbeatErr_t cmr_canFSMHeartbeatErr_t cmr_canHV_BMSHeartbeatErr_t  /**< @brief Error matrix. */ 
    uint8_t warning[2];    //Flag:cmr_canVSMHeartbeatWrn_t cmr_canVCUHeartbeatWrn_t cmr_canFSMHeartbeatWrn_t  /**< @brief Warning matrix. */ 
} cmr_canHeartbeat_t;
...
//     CMR_CAN_ERROR_PTC_WATER_TEMP = (1 << 13),
    ...
    /** @brief VCU All motor controllers have errored or timed out. */
    CMR_CAN_ERROR_VCU_DTI_ALL = (1 << 15)
} cmr_canError_t;
...
typedef enum {
    ...
    /** @brief VSM hasn't received VCU heartbeat for 25 ms. */
    CMR_CAN_WARN_VSM_VCU_TIMEOUT = (1 << 13),
    ...
    /** @brief VCU Front left motor controller is warning source. */
    CMR_CAN_WARN_VCU_DTI_FL = (1 << 15),
    /** @brief VCU Front right motor controller is warning source. */
    CMR_CAN_WARN_VCU_DTI_FR = (1 << 14),
    /** @brief VCU Rear left motor controller is warning source. */
    CMR_CAN_WARN_VCU_DTI_RL = (1 << 13),
    /** @brief VCU Rear right motor controller is warning source. */
    CMR_CAN_WARN_VCU_DTI_RR = (1 << 12),
    /** @brief VCU Motor controller has an error. */
    CMR_CAN_WARN_VCU_DTI_ERROR = (1 << 11),
    /** @brief VCU Motor controller has timed out. */
    CMR_CAN_WARN_VCU_DTI_TIMEOUT = (1 << 10),
    CMR_CAN_WARN_VCU_MEMORATOR_DAQ_TIMEOUT = (1 << 9)
} cmr_canWarn_t;
...
typedef enum {
    ...
    CMR_CAN_VSM_TIMEOUT_SOURCE_VCU = (1 << 5),
...
typedef enum {
    ...
    /** @brief At least one Central Dynamics Controller message has timed out. */
    CMR_CAN_VSM_BADSTATE_SOURCE_VCU = (1 << 5),
    /** @brief At least one Front Sensor Module message has timed out. */
...
typedef enum {
    ...
    CMR_CAN_VSM_WRN_VCU_TIMEOUT = (1<<13),
...
// ------------------------------------------------------------------------------------------------
// Vehicle Controls Unit (27x)

typedef enum{
    CMR_CAN_VCU_ERR_VSM_TIMEOUT = (1<<0),
    CMR_CAN_VCU_ERR_DTIALLERROR = (1<<15) 
} cmr_canVCUHeartbeatErr_t; 

typedef enum {
    CMR_CAN_VCU_WRN_VSM_TIMEOUT = (1<<0),
    CMR_CAN_VCU_WRN_MEMORATOR_TIMEOUT  = (1<<9), 
    CMR_CAN_VCU_WRN_DTI_TIMEOUT = (1<<10), 
    CMR_CAN_VCU_WRN_DTI_ERROR = (1<<11),
    CMR_CAN_VCU_WRN_DTI_SRC_RR = (1<<12), 
    CMR_CAN_VCU_WRN_DTI_SRC_RL = (1<<13),
    CMR_CAN_VCU_WRN_DTI_SRC_FR = (1<<14),
    CMR_CAN_VCU_WRN_DTI_SRC_FL = (1<<15)
} cmr_canVCUHeartbeatWrn_t; 
...
} cmr_canVCUSafetyFilterStates_t;
...
/** @brief VCU wheel speeds (used for setpoint and actual). */
typedef struct {
