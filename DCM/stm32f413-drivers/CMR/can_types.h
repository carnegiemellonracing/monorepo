...
typedef struct {
    float guessed_vel;             /**< @brief Velocity of previous timestep (m / s). */
    float estimated_velocity;      /**< @brief ESTIMATED VELOCITY (m / s). */
} cmr_canCDCVelocityEstimation1_t;

typedef struct {
    float aero_drag;               /**< @brief Aero drag based on previous velocity (N). */
    float aero_lift;               /**< @brief Aero lift based on previous velocity (N). */
} cmr_canCDCVelocityEstimation2_t;
...
typedef struct
{
   float angle_whl_fl;
   float angle_whl_fr;
} cmr_can_front_whl_slip_angle_t;

typedef struct
{
    float moment_req_Nm;
    float lin_accel_Nm;
} cmr_can_CVXGEN_info_t;

typedef struct {
    uint32_t non_convergence_count;
} cmr_can_CVXGEN_counter_t;
