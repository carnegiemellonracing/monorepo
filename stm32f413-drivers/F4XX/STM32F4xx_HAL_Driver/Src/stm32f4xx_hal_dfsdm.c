...
void HAL_DFSDM_IRQHandler(DFSDM_Filter_HandleTypeDef *hdfsdm_filter)
{
...
#if (USE_HAL_DFSDM_REGISTER_CALLBACKS == 1)
    a_dfsdm1ChannelHandle[channel]->SDCMallback(a_dfsdm1ChannelHandle[channel]);
#else
    HAL_DFSDM_ChannelSDCMallback(a_dfsdm1ChannelHandle[channel]);
#endif
  }
#if defined (DFSDM2_Channel0)
  /* Check if short circuit detection occurs */
         ...
         ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_SCDIE) != 0U))
  {
    ...
    /* Call short circuit detection callback */
#if (USE_HAL_DFSDM_REGISTER_CALLBACKS == 1)
    a_dfsdm2ChannelHandle[channel]->SDCMallback(a_dfsdm2ChannelHandle[channel]);
#else
    HAL_DFSDM_ChannelSDCMallback(a_dfsdm2ChannelHandle[channel]);
#endif
  }
#endif /* DFSDM2_Channel0 */
}
